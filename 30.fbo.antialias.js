...
const createFramebufferObject = (program) => {
	...
	gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.RENDERBUFFER, positionRenderbufferMSAA);
	gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT1, gl.RENDERBUFFER, normalRenderbufferMSAA)
	gl.framebufferRenderbuffer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.RENDERBUFFER, depthRenderbufferMSAA);

	// We'll be populating the attachment by DRAWING to them. So we need to tell WebGL to fill both color attachments when drawing:
	...
	// Also, the attached textures will be populated by blitting now, not drawing. So we can remove this line:
	// gl.drawBuffers([ gl.COLOR_ATTACHMENT0, gl.COLOR_ATTACHMENT1 ]);
...
// Create the two framebuffer objects:
//  - fboMSSA has the multisample renderbuffers
//  - fbo has the regular textures
const [fboMSAA, fbo] = createFramebufferObject(quad.program);

const animate = () => {
	lights.update();

	// 1. Draw geometry to multisample FBO
	gl.bindFramebuffer(gl.FRAMEBUFFER, fboMSAA);
	geometry.draw();
	gl.bindFramebuffer(gl.FRAMEBUFFER, null);
	...
	// 2a. Select the source & destination FBOs
	gl.bindFramebuffer(gl.READ_FRAMEBUFFER, fboMSAA); // Source
	gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER, fbo);     // Destination

	// 2b. Select source & destination color attachments for the position data, then blit the data
	gl.readBuffer(gl.COLOR_ATTACHMENT0);
	gl.drawBuffers([gl.COLOR_ATTACHMENT0]);
	gl.blitFramebuffer(0,0,480,480,  0,0,480,480,  gl.COLOR_BUFFER_BIT, gl.NEAREST);
	
	// 2c. Select source & destination color attachments for the normal data, then blit the data
	gl.readBuffer(gl.COLOR_ATTACHMENT1);
	gl.drawBuffers([gl.NONE, gl.COLOR_ATTACHMENT1]);
	gl.blitFramebuffer(0,0,480,480,  0,0,480,480,  gl.COLOR_BUFFER_BIT, gl.NEAREST);

	// 2d. Clean up after blit is finished but before the next draw call.
	gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER, null);
	gl.bindFramebuffer(gl.READ_FRAMEBUFFER, null);

	// 3. No framebuffer is bound, so the data will be sent to the canvas
	quad.draw();

	// 4. Set up next animation frame
	requestAnimationFrame(animate);
};

// Initiate first draw call
animate();